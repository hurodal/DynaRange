...
void ArgumentManager::RegisterAllArguments()
{
    ...
    m_descriptors["patch-ratio"]
        = { "patch-ratio", "r", _("Relative patch width/height used to compute signal and noise readings (default=0.5)"), ArgType::Double, DEFAULT_PATCH_RATIO, false, 0.0, 1.0 };
    m_descriptors["snrthreshold-db"]
        = { "snrthreshold-db", "d", _("SNR threshold in dB for DR calculation (default=12dB (photo DR) plus 0dB (engineering DR))"), ArgType::DoubleVector, std::vector<double> { 12.0, 0.0 } };
...
void ArgumentManager::ParseCli(int argc, char* argv[])
{
    ...
    if (chart_colour_opt->count() > 0)
        m_values["chart-colour"] = temp_opts.chart_colour_params;
    if (chart_patches_opt->count() > 0)
        m_values["chart-patches"] = temp_opts.chart_patches;
    if (chart_coords_opt->count() > 0)
        m_values["chart-coords"] = temp_opts.chart_coords;
...
ProgramOptions ArgumentManager::ToProgramOptions()
{
    ...
    if (opts.generate_plot) {
        std::string format_str = Get<std::string>("plot-format");
        ...
        auto params_vec = Get<std::vector<int>>("plot-params");

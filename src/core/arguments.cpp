...
ProgramOptions ParseArguments(int argc, char* argv[]) {
    ...
    app.add_option("--poly-fit,-f", opts.poly_order, _("Polynomic order (default=2) to fit the SNR curve"))->check(CLI::IsMember({2, 3, 4, 5}))->default_val(2);
    ...
    // Ya no se procesan los ficheros aquí, solo se devuelven las opciones.
    return opts;
}
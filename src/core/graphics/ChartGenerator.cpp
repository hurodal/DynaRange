...
// It contains the core Cairo drawing logic shared by both public functions.
    ...
    for (int row = 0; row < NROWS; ++row) {
        for (int col = 0; col < NCOLS; ++col) {
            ...
            cairo_set_source_rgb(cr, (intensity * opts.R) / RGBMAX, (intensity * opts.G) / RGBMAX, (intensity * opts.B) / RGBMAX);
...
bool GenerateTestChart(
    ...
    if (!surface) {
        log_stream << _("Error: Invalid chart parameters or Cairo context creation failed.") << std::endl;
        return false;
    }

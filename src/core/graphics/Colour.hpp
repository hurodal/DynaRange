...
namespace PlotColors {
    ...
    // Debug Colors
    constexpr double YELLOW[] = {1.0, 1.0, 0.0}; // Vibrant yellow for selection/debug
    constexpr double MAGENTA[] = {1.0, 0.0, 1.0}; // Vibrant magenta for debug
    ...
    inline void cairo_set_source_yellow(cairo_t* cr) { cairo_set_source_rgb(cr, YELLOW[0], YELLOW[1], YELLOW[2]); }
    inline void cairo_set_source_magenta(cairo_t* cr) { cairo_set_source_rgb(cr, MAGENTA[0], MAGENTA[1], MAGENTA[2]); }

...
std::optional<double> ProcessDarkFrame(const std::string& filename, std::ostream& log_stream) {
    log_stream << _("Calculating black level from: ") << filename << "..." << std::endl;
    ...
    log_stream << _("Black level obtained: ") << std::fixed << std::setprecision(2) << mean_value << std::endl;
    return mean_value;
}

std::optional<double> ProcessSaturationFrame(const std::string& filename, std::ostream& log_stream) {
    log_stream << _("Calculating saturation point from: ") << filename << "..." << std::endl;
    ...
    log_stream << _("Saturation point obtained (5th percentile): ") << std::fixed << std::setprecision(2) << quantile_value << std::endl;
...
std::optional<double> OldProcessSaturationFrame(const std::string& filename, std::ostream& log_stream) {
    ...
    int bit_depth = 0;
    if (bit_depth == 0) {
        log_stream << _("[WARNING] Could not determine bit depth from RAW metadata. Assuming 14-bit.")
                   << std::endl;
        bit_depth = 14;
    }
    ...
    log_stream << _("Detected bit depth: ") << bit_depth << _(" bits") << std::endl;
    log_stream << _("Expected saturation level: ") << expected_saturation << std::endl;
    log_stream << _("Measured saturation (5th percentile): ") << std::fixed << std::setprecision(2) << quantile_value << std::endl;

    // If measured value is suspiciously low (e.g., ~4095 for 14-bit), warn user
    if (bit_depth == 14 && quantile_value < 10000) {
        log_stream << _("[WARNING] Measured saturation (") << quantile_value << _(") is much lower than expected (") << expected_saturation << _("). ")

...
double calculate_dr_for_threshold(double threshold, const std::vector<double>& snr_db, const std::vector<double>& signal_ev, int poly_order, std::ostream& log_stream) {
...
bool run_dynamic_range_analysis(const ProgramOptions& opts, std::ostream& log_stream) {
    ...
    std::vector<CurveData> all_curves_data; // AÑADIDO: Vector para guardar los datos de todas las curvas
    Eigen::VectorXd k;
    ...
    for (int i = 0; i < filenames.size(); ++i) {
        ...
        generate_snr_plot(plot_path.string(), fs::path(name).filename().string(), signal_ev, snr_db, poly_coeffs_global);

        double dr_12db = calculate_dr_for_threshold(12.0, snr_db, signal_ev, opts.poly_order, log_stream);
        double dr_0db = calculate_dr_for_threshold(0.0, snr_db, signal_ev, opts.poly_order, log_stream);
    ...
    if (!all_curves_data.empty()) {
        ...
        generate_summary_plot(summary_plot_path.string(), all_curves_data);
    }

    log_stream << "\n--- Dynamic Range Results ---\n";
    log_stream << std::left 
              << std::setw(30) << "RAW File"

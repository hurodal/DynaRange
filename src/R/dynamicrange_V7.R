...
point_colour=c('red', 'green', 'darkgreen', 'blue', 'black')  # R, G1, G2, B, ALL colours
...
# 2. EXTRACT INDIVIDUAL RAW CHANNEL(S) AND APPLY KEYSTONE CORRECTION
    ...
    for (rawchan in 1:4) {
        cat(paste0('  Reading RAW channel: ', rawchan, '...\n'))
...
# 3. READ PATCHES TO FORM GRID AND COLLECT (EV,SNR) PAIRS
    ...
    for (rawchan in 1:4) {
        ...
        if (image==1 && rawchan==1) {
            ...
            abline(v=seq(-20,0,1), lty=2, col='gray')
            axis(side=1, at=-20:0)
        } else {
...
# 4. APPROXIMATION CURVES TO CALCULATE DR VALUES
    ...
    for (rawchan in 1:5) {
        ...
        text(max(valuesx), max(valuesy)+0.5, labels=filenamesISO[image], col='magenta')

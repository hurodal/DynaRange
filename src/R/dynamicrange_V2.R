...
ZOOM=1
CairoPNG("SNRcurvesBLACK254.85.png", width=1920*ZOOM, height=1080*ZOOM)  # HQ Full HD curves
...
# 3. READ PATCHES TO FORM 7x11 GRID AND COLLECT (EV,SNR) PAIRS
    ...
    # SNR cuves in dB
    if (image==1) {
        plot(log2(Signal), 20*log10(SNR), xlim=c(-15,0), ylim=c(-15,25),
...
text(-15+0.1, 12+0.5, labels="Photographic DR (SNR>12dB)", adj=0)
text(-15+0.1,  0+0.5, labels="Engineering DR (SNR>0dB)", adj=0)
